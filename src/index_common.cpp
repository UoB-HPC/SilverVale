...
agv::resolveProgramAndDetect(const std::filesystem::path &program,
  ...
  if (auto code = agv::exec(fmt::format("{} --version", program), output); !code || *code != 0)
    return {};
...
std::string agv::readFile(const std::filesystem::path &file) {
  std::ifstream t(file);
...
std::vector<std::string> agv::stripDashOArgs(const std::vector<std::string> &args) {
  return args                                                                               //
         | zip_with_index()                                                                 //
...
std::map<std::string, agv::Dependency> agv::readDepFile(const std::filesystem::path &depFile,
                                                        const std::string &source) {
  std::map<std::string, agv::Dependency> deps;
  auto addDep = [&](const std::filesystem::path &file) {
    try {
      auto time = std::chrono::system_clock::to_time_t(       //
          std::chrono::clock_cast<std::chrono::system_clock>( //
              std::filesystem::last_write_time(file)));
      deps.emplace(file, agv::Dependency{time, readFile(file)});
